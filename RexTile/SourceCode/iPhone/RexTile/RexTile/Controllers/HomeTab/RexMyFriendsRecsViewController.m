//
//  RexMyFriendsRecsViewController.m
//  RexTile
//
//  Created by Sweety Singh on 1/6/15.
...
@interface RexMyFriendsRecsViewController ()<WebServiceDelegate, UITableViewDataSource, UITableViewDelegate, MKMapViewDelegate, UISearchBarDelegate,iCarouselDataSource,iCarouselDelegate>


@property (nonatomic, strong) RecommendationListWebServices *recommendationWebService;
@property (nonatomic, strong) NSMutableArray *friendsRecsArray;
...
@implementation RexMyFriendsRecsViewController
{
     CGFloat yOffset;
    BOOL isSearching;
...
-(void)fetchMyFriendsRecs{
    ...
    [self.recommendationWebService getMyFriendsRecs];
}
...
-(NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section{
    if (isSearching) {
        ...
        return [self.friendsRecsArray count];
    }
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (isSearching) {
        ...
        placeModel = [self.friendsRecsArray objectAtIndex:indexPath.row];
    }
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (isSearching) {
        ...
        placeDetailView.placesArray = self.friendsRecsArray;
    }
...
- (void)searchBarTextDidBeginEditing:(UISearchBar *)searchBar{
    ...
    if(searchBar.text.length == 0)
    {
        ...
        [self.filteredRecsArray addObjectsFromArray:self.friendsRecsArray];
    }
    [self addTapGesture];
...
- (void)searchBar:(UISearchBar *)searchBar textDidChange:(NSString *)searchText{
    ...
    else
    {
        ...
        [self.filteredRecsArray removeAllObjects];
        [self.tableView setFrame:CGRectMake(20, 200, self.tableView.frame.size.width, self.tableView.frame.size.height)];
        for (GooglePlaceModel *recModel in self.friendsRecsArray)
        {
...
-(void)request:(id)asynchroniousRequest didFinishWithResult:(NSArray *)result{
    ...
    switch ([asynchroniousRequest tag])
    {
        case 1://My Recs
        {
            ...
            self.friendsRecsArray = [NSMutableArray arrayWithArray:result];
            [self.tableView reloadData];
